...
if(localStorage.getItem('token') === token){
	 msgShow0 += `<div class="flex gap-4 items-center justify-end"><div class="bg-blue-500 flex gap-2 rounded-sm"><span id="showUser" class="flex items-center pl-2 bg-gray-300 pr-2 font-bold text-xs">${user} </span><span class="flex text-white pr-2 items-center text-sm">${msg}</span></div></div>`;
	} else {
